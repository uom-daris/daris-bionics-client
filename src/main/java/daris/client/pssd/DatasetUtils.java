...
public class DatasetUtils {
    ...
    public static String uploadDataset(ServerClient.Connection cxn, String pid, String studyName,
            Collection<String> inputCids, boolean derived, String datasetName, String datasetDescription, File f,
            boolean gzip, ArchiveType archiveType, String type, ProgressMonitor pm) throws Throwable {
        ...
        if (pid == null) {
            throw new IllegalArgumentException("No study parent cid or study cid is specified. Cannot find study.");
        }
        String studyCid = StudyUtils.findStudy(cxn, pid, studyName);
        if (studyCid == null) {
            throw new IllegalArgumentException(
                    "Cannot find study. (pid='" + pid + "',  study name='" + studyName + "')");
        }
        if (!derived) {
            if (inputCids != null) {
                if (!inputCids.isEmpty()) {
                    throw new IllegalArgumentException("Unexpected input cids when creating primary dataset.");
                }
            }
        }
        ...
        datasetCid = createDataset(cxn, studyCid, inputCids, derived, datasetName, datasetDescription, f, gzip,
                archiveType, type, pm);
        return datasetCid;
    }

    static String createDataset(Connection cxn, String studyCid, Collection<String> inputCids, boolean derived,
            String datasetName, String datasetDescription, File f, boolean gzip, ArchiveType archiveType, String type,
            ProgressMonitor pm) throws Throwable {
        if (derived) {
            return createDerivedDataset(cxn, studyCid, datasetName, datasetDescription, inputCids, f, archiveType, gzip,
                    type, pm);
        } else {
            return createPrimaryDataset(cxn, studyCid, datasetName, datasetDescription, f, archiveType, gzip, type, pm);
        }
    }

    private static String createPrimaryDataset(Connection cxn, String studyCid, String datasetName,
            String datasetDescription, File f, ArchiveType archiveType, boolean gzip, String type, ProgressMonitor pm)
            throws Throwable {

        XmlStringWriter w = new XmlStringWriter();
        addDatasetMeta(cxn, studyCid, datasetName, datasetDescription, f, type, archiveType, gzip, w);
        w.push("subject");
        ...
        return cxn.execute("om.pssd.dataset.primary.create", w.document(), input, null).value("id");
    }

    private static String createDerivedDataset(Connection cxn, String studyCid, String datasetName,
            String datasetDescription, Collection<String> inputCids, File f, ArchiveType archiveType, boolean gzip,
            String type, ProgressMonitor pm) throws Throwable {
    ...
    private static void addInputDataset(ServerClient.Connection cxn, String inputCid, XmlStringWriter w)
            throws Throwable {
        XmlDoc.Element ae = ObjectUtils.getAssetMeta(cxn, inputCid);
        w.add("input", new String[] { "vid", ae.value("@vid") }, inputCid);
    }

    private static void addDatasetMeta(ServerClient.Connection cxn, String studyCid, String datasetName,
            String datasetDescription, File f, String type, ArchiveType archiveType, boolean gzip, XmlStringWriter w)
            throws Throwable {
    ...
    private static Input createInputFromDirectory(final File dir, final ArchiveType archiveType,
        ...
        return new ServerClient.GeneratedInput(archiveType.mimeType(), archiveType.fileExtension(),
            ...
            protected void copyTo(OutputStream out, AbortCheck ac) throws Throwable {
                ...
                ArchiveOutput ao = ArchiveRegistry.createOutput(out, archiveType.mimeType(), 6, null);
                try {
    ...
    static String findDataset(ServerClient.Connection cxn, String pid, String studyName, String datasetName, File f)
        ...
        if (studyName != null) {
            query.append(" and (cid contained by(xpath(daris:pssd-object/name)='" + studyName + "'))");
        }
        if (datasetName != null) {
